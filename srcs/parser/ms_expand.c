...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/18 12:00:00 by mbyrne            #+#    #+#             */
/*   Updated: 2025/03/18 12:00:00 by mbyrne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *expand_tilde(t_mini *mini, char *str)
{
    ...
    // Only expand ~ if it's alone or followed by /
...
char *expand_token(t_mini *mini, t_token *token)
{
    ...
    // Don't expand anything in single quotes
    if (token->token == TOKEN_SINGLE_Q_STRING)
        return (ft_strdup(token->string));

    // First expand tilde if applicable
