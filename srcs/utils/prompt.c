...
/*   Updated: 2025/03/16 15:12:43 by mbyrne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char *create_user_prefix(char *user)
{
    ...
    if (!user)
        return (NULL);
    host = get_hostname();
    temp = ft_str_join(user, "@");
    prefix = ft_str_join(temp, host);
    free(temp);
    free(host);
    temp = ft_str_join(prefix, ":");
    free(prefix);
    return (temp);
}
...
static char *format_prompt(char *prefix, char *path)
{
    ...
    if (prefix)
    {
        temp = ft_str_join(prefix, path);
        prompt = ft_str_join(temp, "$ ");
        free(temp);
    }
    else
